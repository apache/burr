...
def test_persister_methods_none_partition_key(
    persistence, method_name: str, kwargs: dict
):
    if hasattr(persistence, "initialize"):
        persistence.initialize()
...
async def test_AsyncSQLitePersister_from_values():
    ...
    sqlite_persister_init = AsyncSQLitePersister(
        connection=connection, table_name="test_table"
    )
...
async def test_asyncsqlite_persistence_is_initialized_true_new_connection(tmp_path):
    ...
    await p.initialize()
    p2 = await AsyncSQLitePersister.from_values(
...
async def test_async_save_and_load_from_sqlite_persister_end_to_end(tmp_path):
    ...
    app = await (
        ...
        .with_transitions(
            ("dummy_input", "dummy_response"), ("dummy_response", "dummy_input")
        )
        .initialize_from(
